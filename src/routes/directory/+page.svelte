<script lang="ts">
  ...
  let kind = $state<DirectoryKind>(page.url.searchParams.get('kind') === 'officers' ? 'officers' : 'charities');
  let letter = $state((page.url.searchParams.get('letter') ?? 'A').toUpperCase().match(/^[A-Z]$/) ? (page.url.searchParams.get('letter') ?? 'A').toUpperCase() : 'A');
  let sort = $state(['most', 'least'].includes(page.url.searchParams.get('sort') ?? '') ? page.url.searchParams.get('sort')! : 'az');
  let pageNum = $state(Math.max(1, Number(page.url.searchParams.get('page') ?? 1) || 1));
  ...
  async function load() {
    loading = true;
    const params = new URLSearchParams({ kind, letter, sort, page: String(pageNum), pageSize: String(pageSize) });
  ...
  function choose(nextKind: DirectoryKind, nextLetter: string, nextSort = sort) {
    ...
    sort = nextSort;
    pageNum = 1;
    goto(buildUrl(1), { replaceState: true, noScroll: true });
    load();
  }

  function goToPage(nextPage: number) {
    pageNum = Math.max(1, Math.min(nextPage, totalPages));
    goto(buildUrl(pageNum), { replaceState: true, noScroll: true });
    load();
  }
...
<header>
  <div>
    <a class="brand" href="/">NZ Records</a>
    <nav>
      <a href="/explore">Search</a>
      <a href="/directory?kind=charities&letter=A">Charities</a>
      <a href="/directory?kind=officers&letter=A">Officer records</a>
    </nav>
  </div>
</header>
