<script lang="ts">
  ...
  async function runSearch(nextPage = pageNum) {
    const q = query.trim();
...
<main><p class="breadcrumb"><a href="/">Home</a> / Search</p><h1>Search public records</h1><p class="intro">Search names, organisations and identifiers in the NZ Records database.</p><label class="search"><span>Search</span><div><input bind:value={query} oninput={syncQuery} placeholder="Name, organisation or NZBN"/><button onclick={() => { pageNum = 1; goto(buildUrl(1), { replaceState: true, noScroll: true }); runSearch(1); }}>Search</button></div></label>
<div class="summary">{summaryText()}{#if mode === 'demo'}<span>Demo data</span>{/if}</div><section>{#each results as entity}{@const id=entity.id}{@const name=entity.canonical_name??entity.name??'Unnamed record'}{@const type=entity.entity_type??entity.type??'other'}{@const subtitle=entity.subtitle??[type.replace('_',' '),entity.status,entity.nzbn?`NZBN ${entity.nzbn}`:null].filter(Boolean).join(' · ')}{@const connections=Number(entity.relationship_count??0)}<a class="result" href={mode==='demo'?`/entity/${id}`:`/record/${id}`}><div><strong>{name}</strong><span>{subtitle}</span></div>{#if mode==='live'}<div class="connections"><b>{connections}</b><span>{connections===1?'connection':'connections'}</span></div>{/if}<span class="view">View record</span></a>{/each}{#if !loading&&results.length===0}<div class="empty">No records match your search.</div>{/if}</section>
